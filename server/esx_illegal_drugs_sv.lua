...
local function SellWeed(source)
	...
	SetTimeout(Config.TimeToSellWeed, function()
        ...
        -- local bigCop = CopsConnected - 4
        -- local priceBase = Config.WeedPriceBase
        -- local priceCopMult = Config.PriceCopMult
        -- local priceBigCopMult = Config.PriceBigCopMult
        -- local totalPrice = 0
        -- if (CopsConnected >= 3 and CopsConnected <= 7) then
        --   totalPrice = priceBase + (CopsConnected * priceCopMult)
        -- else
        --   totalPrice = priceBase + (CopsConnected * priceCopMult) + (bigCop * priceBigCopMult)
        -- end
...
local function SellOpium(source)
	...
	SetTimeout(Config.TimeToSellOpium, function()
        ...
        -- local bigCop = CopsConnected - 4
        -- local priceBase = Config.OpiumPriceBase
        -- local priceCopMult = Config.PriceCopMult
        -- local priceBigCopMult = Config.PriceBigCopMult
        -- local totalPrice = 0
        -- if (CopsConnected >= 3 and CopsConnected <= 7) then
...
local function SellCoke(source)
	...
	SetTimeout(Config.TimeToSellCoke, function()
        ...
        -- local bigCop = CopsConnected - 4
        -- local priceBase = Config.CokePriceBase
        -- local priceCopMult = Config.PriceCopMult
        -- local priceBigCopMult = Config.PriceBigCopMult
        -- local totalPrice = 0
        -- if (CopsConnected >= 3 and CopsConnected <= 7) then
...
local function SellMeth(source)
	...
	SetTimeout(Config.TimeToSellMeth, function()
        ...
        -- local bigCop = CopsConnected - 4
        -- local priceBase = Config.MethPriceBase
        -- local priceCopMult = Config.PriceCopMult
        -- local priceBigCopMult = Config.PriceBigCopMult
        -- local totalPrice = 0
        -- if (CopsConnected >= 3 and CopsConnected <= 7) then
